...
def mlp_return_strupkl(
    ...
    if next_md_configuration_names is not None:
        result_keys = []
        for configuration_name in next_md_configuration_names:
            configuration_name = str(configuration_name)
            if not configuration_name.endswith('.vasp'):
                configuration_name += '.vasp'
            if configuration_name not in result_keys:
                result_keys.append(configuration_name)
