...
def _example_sample_candidates(root: Path):
    ...
    yield root / "source" / "OCBF" / "example" / "sample"
...
def _resolve_example_sample_dir():
    ...
    for env_name in ("OCBF_ROOT", "OCBF_V3_ROOT"):
...
def handle_create_init_command(_args):
    ...
    if conflicts:
        ...
        raise FileExistsError(
            f"ocbf create-init aborted because these paths already exist in {target_dir}: {conflict_text}"
        )
...
def build_parser(include_advanced_commands=True):
    ...
    mp_search_parser = subparsers.add_parser(
        ...
        epilog=defaults_epilog("mp_search"),
    )
